...
print('number of leaves cart', cart.get_n_leaves())
...
for k in range(len(flipped_ccps)):
	...
	if (cum_pval <= 0.001):
		max_subtree_cum_pvals001 = k
	if (cum_pval <= 0.05):
		max_subtree_cum_pvals05 = k
...
print('index cum pvalue', max_subtree_cum_pvals05)
print(flipped_ccps[max_subtree_cum_pvals05]) 
#index cum pvalue 53
#0.0011744717353133899

#tree.plot_tree(cart1, fontsize = 7, impurity = False, label = 'none', feature_names = feature_names)
msep_T_star_c = mseps[max_subtree_cum_pvals001]
...
plt.axvline(x = max_subtree_cum_pvals05, color = 'darkgreen', label = 'delta = 0.05, cum_pval')
