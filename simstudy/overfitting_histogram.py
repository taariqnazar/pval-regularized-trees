
#! usr/bin/env python3

import numpy as np
import matplotlib.pyplot as plt
from scipy.stats import norm
from sklearn import tree
from sklearn.tree import DecisionTreeRegressor
import utils
#import dtreeviz


#### seed for random generation ####

np.random.seed(3)

#### Hyperparameters ####

N = 1000
delta = .1


# 10-dimensional iid Gaussian
d = 10

X = np.random.normal(0, 1, size = (N, d))


#### Regression functions ####

# adampted Neufeld example with 10 leaves (want more complex tree)
b = 1
def mu(x):
	if(x[0] <= 0):
		if(x[1] <= 0):
			if(x[2] <= 0):
				if(x[3] <= 0):
					return 0*b
				else:
					return 1*b
			else:
				if(x[3] <= 0):
					return b*2
				else:
					return b*3
		else:
			if(x[2] <= 0):
				if(x[4] <= 0):
					return b*4
				else:
					return b*5
			else:
				return 6*b
	else:
		if(x[5] <= 0):
			if(x[1] <= 0):
				return 7*b
			else: 
				return 8*b
		else:
			if(x[1] <= 0):
				return 9*b
			else:
				return 10*b


# Neufeld example
b = 1
a = 2
mu2 = lambda x: b*(1 if x[0] <= 0 else 0)*(1 + a*(1 if x[1] > 0 else 0) + (1 if x[1]*x[2] > 0 else 0))



#### Responses ####

sigma_sq = 1**2
#mus = [mu(x) for x in X]
#Y = np.random.normal(mus, sigma_sq, N)


#### CART tree ####

K = 6
min_dp_leaf = 2
#cart = DecisionTreeRegressor(max_depth=K, min_samples_leaf = min_dp_leaf).fit(X, Y)



###### Simulation #########

Ns = [1000, 10000, 50000]
means = []

M = 1000

for N in Ns:

	number_leaves = []
	mseps = []


	for m in range(M):
		X_m = np.random.normal(0, sigma_sq, size = (N, d))
		mus_m = [mu(x) for x in X_m]
		Y_m = np.random.normal(mus_m, sigma_sq, N)
		tree = DecisionTreeRegressor(max_depth=K, min_samples_leaf = min_dp_leaf).fit(X_m, Y_m)
		n_leaves = len(utils.get_optimal_leaves(tree, delta, d))
		number_leaves.append(n_leaves)
		#utils.prune_to_T_star(tree, delta, d)
		#pred_test = tree.predict(X_test)
		#mseps.append(utils.get_MSE(Y_test, pred_test))


	print('Theoretical Mean:', 11*(1-delta)/(1 - 2*delta))
	print('Mean:', np.mean(number_leaves))
	means.append(np.mean(number_leaves))






#print(mseps)

#print(number_leaves)




#### Plots ####

#plt.plot(Ns, precision_rates)
#plt.hist(mseps, bins = 100)
plt.hist(number_leaves, bins = 10)



# Plot tree

#tree.plot_tree(cart, fontsize = 7, impurity = False, label = 'none')
#features_names = ['X1', 'X2', 'X3', 'X4','X5', 'X6','X7', 'X8', 'X9', 'X10']

# dtreeviz 
'''
viz_rmodel = dtreeviz.model(cart, X, Y, feature_names= features_names,target_name='Y')
v = viz_rmodel.view(depth_range_to_display=(0, 2), scale=2)
v.save('img_url.svg')
'''

plt.show()