...
from src.single_tree import ccp_pval_regularised_tree, train_regression_tree
from src.utils import generate_neufeldt_data
...
def main():
    ...
    for k_folds in [5]:
        ...
        for i in range(n_iterations):
            ...
            alphas_all = []
            for j, (train_indices, validation_indices) in enumerate(
                kf.split(X_train)
            ):
        ...
        plt.savefig(".data/neufeldt_rmse_histogram.pdf", bbox_inches="tight", transparent=True)
        plt.show()
        tree = train_regression_tree(
            X_train,
            y_train,
        ...
        realised_alphas = [get_realiased_ccp(ccp_alpha, alpha) for alpha in unique_alphas]
