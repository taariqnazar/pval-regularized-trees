...
from trees.models import MODEL_REGISTRY
...
_DEFAULT_OUT = Path(__file__).resolve().parents[2] / "results" / "legacy" / "boosting_variants"
_DEFAULT_CFG = Path(__file__).resolve().parent / "boosting_config.yaml"


def run(cfg_path: str | Path = _DEFAULT_CFG, outdir: str | Path = _DEFAULT_OUT):
