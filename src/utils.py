...
def generate_linear_data(n=200, p=10, sigma=5.0, random_state=0):
    """
    Generate linear data, given paramters:
        n,p,sigma,random_state
    """
...
def generate_neufeldt_data(a, b, n=200, p=10, sigma=5.0):
    """
...
def build_ccp_model(X, y, cv_frac=0.5):
    ...
    if cv_frac > 0.0:
        ...
        for ccp in ccp_alphas:
            t_ = DecisionTreeRegressor(
                random_state=0, ccp_alpha=ccp, max_depth=4)
...
def Psi(n, u, d):
    K = np.sqrt(u) - ((2 * (np.log(np.log(n)))) ** (-1 / 2)) * (
        np.log(np.log(np.log(n))) + np.log(2)
    )
...
def get_p_values(cart, d):
    YD_statistics = get_YD_statistics(cart)
    return [
        Psi(cart.tree_.n_node_samples[k], YD_statistics[k], d)
        if (YD_statistics[k] != -1)
        else -1
        for k in range(len(YD_statistics))
    ]
...
def build_our_model(X, y, significance=0.1):
    ...
    ntp = [
        index for index, value in enumerate(get_ps(tree) >= significance / d) if value
    ]
...
def P_T(t, n):
    ...
    K_t = t - a_n * (np.log(np.log(np.log(n))) + np.log(2)) / (
        np.sqrt(2 * np.log(np.log(n)))
    )
    ...
    while len(stack) > 0:
        ...
        if is_split_node:
            ...
            E = n_samples[node_id] * impurities[node_id] - (
                n_samples[children_left[node_id]] *
                impurities[children_left[node_id]]
                + n_samples[children_right[node_id]]
                * impurities[children_right[node_id]]
            )
