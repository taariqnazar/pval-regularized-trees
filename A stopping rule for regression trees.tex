...
\begin{align}
	H^j_0 \colon \mu(x_i) = c \text{ for all } x_i \in D_j \text{ and some } c \in \R.
\end{align}
...
\section{Minimal cost-complexity pruning - Notes and stuff}


- Can we actually calculate the splitting path from Breimans results?
- Breiman uses absolute difference in reduction does this also hold in our
relative case? That is, is there a 1-1 correspondence between his results and
ours?

An observation is that more than one split can be pruned. The reason for this
effect is that each split has the same cost-complexity $ \alpha $ and therefore
the algorithm chooses the smallest tree with a certain $ \alpha $.

For each node we define $\alpha_{eff}(t) = \frac{R(t) - R(T_t)}{size(T_t) - 1}$.
Here $ R(t) $ is the impurity at node $ t $ and $ R(T_t) $ is the impurity of the
tree starting at $ t $. $ R(T_t) $ is the weighted sum of impurities of each
leaf of the tree $ T_t$. So to get the splitting path, we need to calculate
$\alpha_{eff}(t)$ for each node. From this, we can calculate the sorted path of
cost complexities.

In Sklearn, the regression tree has a method called \textbf{decision path} that
returns, the path of a datapoint in the form of a matrix, where the entry $ m \times
	t $ is non-zero if datapoint $ m $ passes through node $ t $. From this, we can
pinpoint the leaf of each node by looking at the index of each non-zero entry.
To calculate the cost-complexities, we need to recalculate the alpha eff after
each prune.

Nils suggested that, we look at single node reduction at each step. That is, we
look at the parents of the lowest nodes and decide if the node should be pruned
or not.
...
\begin{proof}[Proof of Lemma \ref{lem:distr_R}]
	...
	$$
	under $H_0$, i.e. given that $\E[Y_i] = c$ for all $i$ with $x_i \in D = A \dot \cup B$ for some $c \in \R$. For this purpose, we note that for each $r > 0$,
	\begin{align*}
	...
	$$
		\bar \mu^T = (\bar \mu_A^T, \bar \mu_B^T)^T := \left(\mu(x_1), \dots, \mu(x_{n_A}),\mu(x_{n_A + 1}), \dots, \mu(x_n)\right)^T,
	$$
	$$
	...
	\begin{align*}
		\xi_A  = \sum_{i=1}^{n_A -1} b_i^2 =
		  & \frac{1}{\sigma^2} \left(\sum_{i=1}^{n_A} \left(\left(\tilde P_{11}^T \bar \mu_A \right)_i \right)^2 - |(1/\sqrt{n_A}, \dots, 1/\sqrt{n_A}) \mu_A|^2 \right) \\
	...
	\begin{align}\label{eq:Z_law2}
		Z_r \eqdis
		 & \sigma^2 \left(
		\lambda_{A}^r \chi^2_{n_A -1}(\xi_A)
		+ \lambda_{B}^r \tilde \chi^2_{n_B -1}(\xi_B)
	...
	Under $H_0$, $\bar \mu = (c, \dots, c)^T$ for some $c \in \R$. This implies that $\xi_A = \xi_B = \xi_{\text{neg}} = 0$, which is equivalent to $b=0$. It then follows that
	\begin{align}\label{eq:Z_law3}
		...
		\right),
	\end{align}
	where all chi-squares are central and independent. Denoting the random variable in brackets by $Z_r^1$, this means that, under $H_0$,
	$$
